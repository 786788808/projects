...
### 二. 数据探索：  
#### (2.1) 自变量与因变量
自变量：'fixed acidity'固定酸度, 'volatile acidity'挥发性酸度, 'citric acid'柠檬酸, 'residual sugar'残糖, 'chlorides'氯化物, 'free sulfur dioxide'游离二氧化硫, 'total sulfur dioxide'总二氧化硫, 'density'密度,'pH', 'sulphates'硫酸盐, 'alcohol'酒精  
因变量：'quality' 质量分 
#### (2.2) 缺失值情况
数据都是数值型数据，没有缺失值。  
